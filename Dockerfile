...
WORKDIR /workspace
RUN python3 -m venv sglang

WORKDIR /workspace/sglang-guided-decoding
RUN /workspace/sglang/bin/pip install -e "python[all]"
RUN /workspace/sglang/bin/pip install datasets
RUN /workspace/sglang/bin/pip install "vllm==0.9.0.1"

# Setup vllm
COPY ./vllm-guided-decoding /workspace/vllm-guided-decoding

WORKDIR /workspace
RUN python3 -m venv vllm
WORKDIR /workspace/vllm-guided-decoding

RUN SETUPTOOLS_SCM_PRETEND_VERSION=0.0.1 VLLM_USE_PRECOMPILED=1 /workspace/vllm/bin/pip install -e .
RUN /workspace/vllm/bin/pip install "transformers<4.54.0"
RUN /workspace/vllm/bin/pip install datasets
