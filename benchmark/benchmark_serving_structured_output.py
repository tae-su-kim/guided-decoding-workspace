...
def calculate_metrics(
    ...
    for i in range(len(outputs)):
        if outputs[i].success:
            ...
            outputs[i].think_text_len = reasoning_token_len
...
async def benchmark(
    ...
    def prepare_extra_body(request) -> dict:
        ...
        if target_engine_type == "vllm":
            ...
            extra_body["presence_penalty"] = 1.05
            extra_body["chat_template_kwargs"] = {"enable_thinking": reasoning} # for qwen3
    ...
    if profile:
        ...
        profile_input = RequestFuncInput(
            model=model_id,
            prompt=test_request.prompt if not args.dummy_content else " ",
    ...
    async for i, request in get_request(input_requests, request_rate, burstiness):
        ...
        request_func_input = RequestFuncInput(
            model=model_id,
            prompt=request.prompt if not args.dummy_content else " ",
            api_url=api_url,
    ...
    print("{:<40} {:<10}".format("Total reasoning tokens:", metrics.total_reasoning_tokens))
    print(
        "{:<40} {:<10.2f}".format(
    ...
    result = {
        ...
        "reasoning_lens": [output.think_text_len for output in outputs],
        "output_lens": actual_output_lens,
        "ttfts": [output.ttft for output in outputs],
        "tpots": [output.tpot for output in outputs],
        "itls": [output.itl for output in outputs],
        "errors": [output.error for output in outputs],
        "input_tokens": [output.prompt_tokens for output in outputs],
        "output_tokens": [output.output_tokens for output in outputs],
        "reasoning_tokens": [output.reasoning_tokens for output in outputs],
        "arrival_time": [output.arrival_time for output in outputs],
        "queued_ts": [output.queued_ts for output in outputs],
        "scheduled_ts": [output.scheduled_ts for output in outputs],
        "first_token_ts": [output.first_token_ts for output in outputs],
        "last_token_ts": [output.last_token_ts for output in outputs],
